    ...
    public static class MockMetaDataStorageUtil {
        ...
        public static void AddLocalFile(this Mock<IMetaDataStorage> db, IFileInfo path, string id ) {
            var file = Mock.Of<IMappedObject>( f =>
                                              f.RemoteObjectId == id &&
                                              f.LocalSyncTargetPath == path.FullName &&
        ...
        public static Mock<IMappedFolder> AddLocalFolder(this Mock<IMetaDataStorage> db, IDirectoryInfo path, string id ) {
            ...
            folder.Setup(f => f.LocalSyncTargetPath).Returns(path.FullName);
            folder.Setup (f => f.RemoteObjectId).Returns(id);
            db.AddMappedFolder(folder.Object);
            return folder;
        }

        public static void AddMappedFile(this Mock<IMetaDataStorage> db, IMappedObject file) {
            db.Setup( foo => foo.GetObjectByLocalPath(It.Is<IFileInfo>(s => s.FullName == file.LocalSyncTargetPath))).Returns(file);
        ...
        public static void AddMappedFolder(this Mock<IMetaDataStorage> db, IMappedFolder folder) {
            db.Setup( foo => foo.GetObjectByLocalPath(It.Is<IDirectoryInfo>(s => s.FullName == folder.LocalSyncTargetPath))).Returns(folder);
